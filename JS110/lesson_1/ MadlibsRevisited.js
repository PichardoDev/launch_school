...
function performWordSelection(str) {
  Object.keys(words).forEach(wordType => {
    if (str.includes(getPlaceHolder(word))) {
      let randomWordIndex = Math.floor(Math.random() * words[word].length);
      let replacementWord = words[wordType][randomWordIndex];
      str = str.replace(getPlaceHolder(word), randomWordOfTypeIndex);
      console.log(str);
    }
    return str;
  })
